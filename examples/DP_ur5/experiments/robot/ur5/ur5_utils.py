...
def refresh_obs(obs, env):
    ...
    new_obs = env.unwrapped._get_obs()['images']
    new_state = env.unwrapped._get_obs()['state']
    # obs["image_primary"] = new_obs["image_primary"]
    # obs["proprio"] = new_obs["proprio"]
    return new_obs, new_state

def ur5_to_openvla_obs(obs, env):
    # drop obs['state'], obs['wrist']
    obs_dict = {}
    obs_dict['full_image'] = obs['front']
    return obs_dict