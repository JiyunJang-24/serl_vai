...
from serl_launcher.wrappers.front_camera_wrapper import FrontCameraWrapper, FrontCameraLIBEROWrapper, FWBWFrontCameraRewardClassifierWrapper, FWBWFrontCameraBinaryRewardClassifierWrapper, GraspClassifierWrapper,GraspClassifierNoVisionWrapper, GraspClassifierRobosuiteWrapper, GripperPenaltyWrapper, GripperPenaltyUR5Wrapper
...
def actor(
    ...
    while step["fw"] < FLAGS.max_steps:
        ...
        with timer.context("step_env"):
            ...
            # if reward > 0 and next_obs['state']['tcp_pose'][2] < -0.06 and next_obs['state']['tcp_pose'][0] < 0.02 and next_obs['state']['tcp_pose'][0] > -0.02 and next_obs['state']['tcp_pose'][1] < 0.02 and next_obs['state']['tcp_pose'][1] > -0.02:
            if reward > 0 and next_obs['state']['tcp_pose'][2] < -0.06:
                reward = 1.0
                done = True
            else:
                reward = 0.0
            ...
            if done or truncated:
                ...
                print(f"transition from {0} to next task: {next_task_id}")
        ...
        for demo_file in demo_files:
            with open(demo_file, "rb") as f:
                ...
                for traj in trajs:
                    ...
                    # rotation_scale=10.,
                    # torque_scale=10.
                    # traj['observations']['state']['tcp_pose'][:3] = traj['observations']['state']['tcp_pose'][:3] / translation_scale
                    # traj['next_observations']['state']['tcp_pose'][:3] = traj['next_observations']['state']['tcp_pose'][:3] / translation_scale
                    # traj['observations']['state']['tcp_pose'][3:] = traj['observations']['state']['tcp_pose'][3:] / rotation_scale
                    # traj['next_observations']['state']['tcp_pose'][3:] = traj['next_observations']['state']['tcp_pose'][3:] / rotation_scale
                    # traj['observations']['state']['tcp_torque'][:3] = traj['observations']['state']['tcp_torque'][:3] / torque_scale
                    # traj['next_observations']['state']['tcp_torque'][:3] = traj['next_observations']['state']['tcp_torque'][:3] / torque_scale
                    current_gripper_pos = traj['observations']['state']['gripper_state']
                    is_already_open = current_gripper_pos > 0.6  # 완전히 열린 상태 기준 (0.6 근처)
                    is_already_closed = current_gripper_pos < 0.6  # 완전히 닫힌 상태 기준 (0.0 근처)
                    if traj['dones'] != True:
                        traj["rewards"] = np.zeros_like(traj["rewards"], dtype=np.float32)
                    act = traj["actions"][-1]
