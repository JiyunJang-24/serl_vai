...
def actor(
    ...
    while step["fw"] < FLAGS.max_steps or step["bw"] < FLAGS.max_steps:
        ...
        with timer.context("step_env"):
            ...
            #TODO grasp penalty wrapper 하면 이거 수정해야함
    ...
    if FLAGS.learner:
        ...
        for demo_file in demo_files:
            with open(demo_file, "rb") as f:
                ...
                for traj in trajs:
                    # for tra in traj:
                    traj['observations']['state'] = {k: v for k, v in traj['observations']['state'].items() if k in ['tcp_pose', 'robot0_gripper_qpos', 'robot0_gripper_qvel']}
                    traj['next_observations']['state'] = {k: v for k, v in traj['next_observations']['state'].items() if k in ['tcp_pose', 'robot0_gripper_qpos', 'robot0_gripper_qvel']}
