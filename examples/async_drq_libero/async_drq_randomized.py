...
xyz_bounding_box = np.array([[-0.27, -0.1, 0.8], [0.15, 0.36, 1.2]])
...
def check_impossible(env):
    obs = env.env.env.env.env.env.env.env._get_observations()
...
def actor(
    ...
    if FLAGS.eval_checkpoint_step:
        ...
        robot_qpos_init_state = env.env.env.env.env.env.env.env.sim.get_state().flatten()[time_len:time_len+9]
        robot_qvel_init_state = env.env.env.env.env.env.env.env.sim.get_state().flatten()[time_len+qpos_len:time_len+qpos_len+9]
        ...
        with timer.context("step_env"):
            ...
            if check_impossible(env):
                ...
                if env.task_id:
                    ...
                    time_len = 1
                    bw_init_state[time_len:time_len+9] = reset_dict['robot_qpos_init_state'] + np.random.uniform(-0.1, 0.1, 9)
                    bw_init_state[time_len+qpos_len:time_len+qpos_len+9] = reset_dict['robot_qvel_init_state'] + np.random.uniform(-0.1, 0.1, 9)
