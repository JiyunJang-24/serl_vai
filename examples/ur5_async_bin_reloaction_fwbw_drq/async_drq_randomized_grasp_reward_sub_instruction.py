...
id_to_task = {0: "fw"}
TrainerPortMapping = {
    "fw": {"port_number": 6710, "broadcast_port": 6711},
}
...
xyz_bounding_box = np.array([[-0.04, -0.24, 0.8], [0.19, 0.26, 1.05]])
...
def actor(
    ...
    robot_qpos_init_state = env.env.env.env.env.env.env.sim.get_state().flatten()[time_len:time_len+9]
    robot_qvel_init_state = env.env.env.env.env.env.env.sim.get_state().flatten()[time_len+qpos_len:time_len+qpos_len+9]
    reset_dict['robot_qpos_init_state'] = robot_qpos_init_state
    reset_dict['robot_qvel_init_state'] = robot_qvel_init_state
    ...
    while step["fw"] < FLAGS.max_steps:
        ...
        with timer.context("step_env"):
            ...
            next_obs, reward, done, info = env.step(actions)
            if reward >= 0.3:
                instruction = language_id_to_text_feature[env.task_id][1]
            else:
                instruction = language_id_to_text_feature[env.task_id][0]

            next_obs['instruction'] = instruction
            ...
            if check_impossible(env):
                ...
                if env.task_id:
                    ...
                    env.env.env.env.env.env.env.sim.set_state_from_flattened(reset_dict['bw_init_state'])
                    next_obs = env.env.env.env.env.env.observation_()
                    for i in range(15):
