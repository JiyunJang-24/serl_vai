...
xyz_bounding_box = np.array([[-0.27, -0.1, 0.8], [0.14, 0.37, 1.2]])
...
if __name__ == "__main__":
    ...
    # fw_neg_file_name = f"fw_neg_{task_description}_demos_{uuid}.pkl".replace(" ", "_").replace("-", "_")
    # bw_pos_file_name = f"bw_pos_{task_description}_demos_{uuid}.pkl".replace(" ", "_").replace("-", "_")
    # bw_neg_file_name = f"bw_neg_{task_description}_demos_{uuid}.pkl".replace(" ", "_").replace("-", "_")
    fw_pos_file_name = f"fw_pos__demos_{uuid}.pkl".replace(" ", "_").replace("-", "_")
    fw_neg_file_name = f"fw_neg__demos_{uuid}.pkl".replace(" ", "_").replace("-", "_")
    bw_pos_file_name = f"bw_pos__demos_{uuid}.pkl".replace(" ", "_").replace("-", "_")
    bw_neg_file_name = f"bw_neg__demos_{uuid}.pkl".replace(" ", "_").replace("-", "_")
    ...
    while demos_count < demos_needed:
        print("Current the number of task: forward positive {}, forward negative {}, backward positive {}, backward negative {}".format(
            len(forward_pos_trajectories), len(forward_neg_trajectories), len(backward_pos_trajectories), len(backward_neg_trajectories)
        ))
        task_id = input("Enter forward positive(z), forward negative(x), backward positive(c), backward negative(v), save and exit (q): ")
