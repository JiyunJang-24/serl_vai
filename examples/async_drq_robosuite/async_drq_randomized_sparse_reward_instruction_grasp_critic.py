...
from serl_launcher.wrappers.serl_obs_wrappers import SERLObsWrapper, SERLObsLIBEROWrapper, SERLObsRobosuiteWrapper, SERLObsRobosuiteInstructionWrapper, RelativeFrame, Quat2EulerWrapper
...
flags.DEFINE_bool(
    "grasp_divide", False, "divide demo buffer"
)
...
def learner(rng, agent: SACAgentHybridSingleArm, replay_buffer, demo_buffer, bw_demo_buffer=None):
    ...
    def stats_callback(type: str, payload: dict) -> dict:
        ...
        nonlocal env_steps
        nonlocal task_id
        nonlocal train_critic_networks_to_update
        nonlocal train_networks_to_update
        ...
        if wandb_logger is not None:
            ...
            if "task_id" in payload:
                ...
                if FLAGS.grasp_divide:
                    if task_id:
                        train_critic_networks_to_update = frozenset({"critic", "bw_grasp_critic"})
                        train_networks_to_update = frozenset({"critic", "bw_grasp_critic", "actor", "temperature"})
                    else:
                        train_critic_networks_to_update = frozenset({"critic", "fw_grasp_critic"})
                        train_networks_to_update = frozenset({"critic", "fw_grasp_critic", "actor", "temperature"})
                else:
                    train_critic_networks_to_update = frozenset({"critic", "grasp_critic"})
                    train_networks_to_update = frozenset({"critic", "grasp_critic", "actor", "temperature"})
    ...
    if FLAGS.actor:
        ...
        for k, v in id_to_task.items():
            ...
            agent: SACAgentHybridSingleArm = make_sac_pixel_agent_hybrid_single_arm_instruction(
                ...
                grasp_divide=FLAGS.grasp_divide
            )
