...
flags.DEFINE_integer(
    "task_buffer_ratio", 50, "task_buffer_ratio"
)
flags.DEFINE_bool(
    "divide_demo_buffer", False, "divide demo buffer"
)
...
language_id_to_task = {0: {0: "Pick up the bread from the light wood bin.", 1: "Place the bread in the dark wood bin."}, 1: {0: "Pick up the bread from the dark wood bin.", 1: "Place the bread in the light wood bin."}}
language_id_to_text_feature = {0: {0: 0, 1: 1}, 1: {0: 0, 1: 1}}



xyz_bounding_box = np.array([[0.005, -0.12, 0.84], [0.185, 0.13, 0.90]]) #0.21이 넘으면 넘기기
...
bw_init_qpos = np.array([0.03154478, 0.84329149, 0.24863209, -1.83479349, -0.14291221, 2.61831289, -0.45724229])

from flax.core.frozen_dict import freeze, unfreeze

def slice_demo_batch(demo_batch, new_batch_size):
    # 1) FrozenDict → 일반 dict로 변환
    mutable_batch = unfreeze(demo_batch)

    # 2) 배열(Array)인 leaf마다 [0:new_batch_size]로 잘라내기
    #    jax.tree_map을 쓰면 중첩된 dict/tuple 구조도 한 번에 처리 가능합니다.
    sliced = jax.tree_map(lambda arr: arr[:new_batch_size], mutable_batch)

    # 3) (선택) 다시 FrozenDict로 묶기
    return freeze(sliced)
...
def actor(
    ...
    env.set_task_id(0)
    obs, _ = env.reset()
    obs['instruction'] = language_id_to_text_feature[env.task_id][0]
    next_obs, reward, done, info = env.step(dummy_action)
    next_obs['instruction'] = language_id_to_text_feature[env.task_id][0]
    done = False
    ...
    while step["fw"] < FLAGS.max_steps:
        ...
        with timer.context("step_env"):
            ...
            next_obs['instruction'] = language_id_to_text_feature[env.task_id][int(info['grasp_success'])]
...
def learner(rng, agent: SACAgentHybridSingleArm, replay_buffer, demo_buffer, bw_demo_buffer=None):
    """
    ...
    global restart
    global stop_time
    task_buffer_ratio = FLAGS.task_buffer_ratio
    ...
    if bw_demo_buffer is not None:
        ...
        bw_demo_iterator = bw_demo_buffer.get_iterator(
            sample_args={
                "batch_size": FLAGS.batch_size // 2,
                "pack_obs_and_next_obs": True,
            },
    ...
    while update_steps < FLAGS.max_steps:
        ...
        if task_id == 0:
            fw_ratio = task_buffer_ratio/100
            bw_ratio = (1 - task_buffer_ratio/100)
        else:
            fw_ratio = (1 - task_buffer_ratio/100)
            bw_ratio = task_buffer_ratio/100
        ...
        for critic_step in range(FLAGS.critic_actor_ratio - 1):
            with timer.context("sample_replay_buffer"):
                ...
                if bw_demo_buffer is not None:
                    bw_demo_batch = next(bw_demo_iterator)
                    bw_demo_batch = slice_demo_batch(bw_demo_batch, int(bw_ratio*(FLAGS.batch_size // 2)))
                    demo_batch = slice_demo_batch(demo_batch, int(fw_ratio*(FLAGS.batch_size // 2)))
                    demo_batch = concat_batches(demo_batch, bw_demo_batch, axis=0)
...
def main(_):
    ...
    list1 =[]
    for i in language_id_to_task.keys():
        for j in sorted(language_id_to_task[i].keys()):
            list1.append(language_id_to_task[i][j])
    inputs = tokenizer(list1, padding=True, return_tensors="jax")
    outputs = model(**inputs)
    last_hidden_states = outputs.last_hidden_state
    feature = last_hidden_states[:, 0, :]

    language_id_to_text_feature[0][0] = feature[0]
    language_id_to_text_feature[0][1] = feature[1]
    language_id_to_text_feature[1][0] = feature[2]
    language_id_to_text_feature[1][1] = feature[3]
    ...
    if FLAGS.learner:
        ...
        if FLAGS.divide_demo_buffer:
            bw_demo_buffer = MemoryEfficientReplayBufferDataStore(
                env.observation_space,
                env.action_space,
                capacity=10000,
                image_keys=image_keys,
            ...
            with open(demo_file, "rb") as f:
                ...
                for traj in trajs:
                    for tra in traj:
                        ...
                        if tra['grasp_success']:
                            instruction = language_id_to_text_feature[task_idx][1]
                        else:
                            instruction = language_id_to_text_feature[task_idx][0]
                        tra['observations']['instruction'] = instruction
                        tra['next_observations']['instruction'] = instruction
                        tra['actions'][3:6] = np.clip(tra['actions'][3:6], 0.0, 0.0)
                        ...
                        if FLAGS.divide_demo_buffer:
                            if "fw" in demo_file:
                                ...
                                bw_demo_buffer.insert(tra)
                        else:
                            demo_buffer.insert(tra)
            if FLAGS.divide_demo_buffer:
                print(f"fw Loaded {demo_file}, buffer size: {len(demo_buffer)}")
                print(f"bw Loaded {demo_file}, buffer size: {len(bw_demo_buffer)}")

            print(f"Loaded {demo_file}, buffer size: {len(demo_buffer)}")

        if FLAGS.divide_demo_buffer:
            print(f"FW Total demo buffer size: {len(demo_buffer)}")
            print(f"BW Total demo buffer size: {len(bw_demo_buffer)}")
