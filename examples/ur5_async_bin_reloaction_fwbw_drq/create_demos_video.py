...
if __name__ == "__main__":
    ...
    parser.add_argument("--demo-file", type=str, required=False, help="Name of the demo file (e.g., bw_pick~.pkl)")
    parser.add_argument("--classifier", type=int, required=False, help="whether the demo is for classifier")
    args = parser.parse_args()

    # Define paths
    
    # demo_load_path = os.path.join("demos_from_mac", args.demo_file)
    demo_load_path = os.path.join("bc_demos/fw_pos", "fw_pos_bc_demos_2025_07_25_19_32_01.pkl")
    ...
    video_wrist_frames = []  # 이미지 프레임 저장 리스트
    video_front_frames = []  # 이미지 프레임 저장 리스트
    for trajectory in tqdm(demo_data, desc="Replaying demos"):
        wrist_image = trajectory["observations"]["wrist"]
        front_image = trajectory["observations"]["front"]
        ...
        video_wrist_frames.append(wrist_image)
        video_front_frames.append(front_image)
            # obs = next_obs
            # if done == 1:
        ...
        # rotated_image = np.rot90(obs['agentview_image'].squeeze(0), k=2)
    # save_video(video_wrist_frames, "video_wrist_video.mp4")
    save_video(video_front_frames, "video_front_video.mp4")
