...
def actor(
    ...
    env.set_task_id(0)
    obs, _ = env.reset(task_id=0)
    obs['subgoal'] = np.array([0])
    next_obs, reward, done, info = env.step(dummy_action)
    next_obs['subgoal'] = np.array([0])
    done = False
    ...
    while step["fw"] < FLAGS.max_steps or step["bw"] < FLAGS.max_steps:
        ...
        with timer.context("step_env"):
            ...
            next_obs, reward, done, info = env.step(actions)
            next_obs['subgoal'] = np.array([int(reward > 0)])
...
def main(_):
    ...
    image_keys = [key for key in env.observation_space.keys() if key != "state" and key != "subgoal"]
    if FLAGS.actor:
        front_image_keys = [
            k for k in env.front_observation_space.keys() if "state" not in k and "subgoal" not in k
        ]
        wrist_image_keys = [
    ...
    if FLAGS.learner:
        ...
        for demo_file in demo_files:
            with open(demo_file, "rb") as f:
                ...
                for traj in trajs:
                    for tra in traj:
                        if prev_reward <= 0 and tra['rewards'] > 0:
                            tra['observations']['subgoal'] = np.array([0])
                            tra['next_observations']['subgoal'] = np.array([1])
                        elif prev_reward > 0 and tra['rewards'] > 0:
                            tra['observations']['subgoal'] = np.array([1])
                            tra['next_observations']['subgoal'] = np.array([1])
                        else:
                            tra['observations']['subgoal'] = np.array([0])
                            tra['next_observations']['subgoal'] = np.array([0])
                        prev_reward = tra['rewards']
