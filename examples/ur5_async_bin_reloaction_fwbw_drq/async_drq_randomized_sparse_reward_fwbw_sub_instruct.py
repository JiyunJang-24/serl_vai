...
from serl_launcher.wrappers.serl_obs_wrappers import SERLObsWrapper, SERLObsInstructWrapper, SERLObsLIBEROWrapper, SERLObsRobosuiteWrapper, RelativeFrame, Quat2EulerWrapper, ScaleObservationWrapper
...
flags.DEFINE_boolean("grasp_reward", False, "Grasp reward.")
...
language_id_to_task = {0: {0: "Pick up the cat doll from the light wood bin.", 1: "Place the the cat doll in the dark wood bin."}, 1: {0: "Pick up the cat doll from the dark wood bin.", 1: "Place the cat doll in the light wood bin."}}
...
def actor(
    ...
    while step["fw"] < FLAGS.max_steps or step["bw"] < FLAGS.max_steps:
        ...
        with timer.context("step_env"):
            ...
            next_obs, reward, done, info = env.step(actions)
            next_obs['instruction'] = language_id_to_text_feature[env.task_id][int(reward > 0)]
            ...
            truncated = (current_step == FLAGS.max_traj_length)
...
def main(_):
    ...
    if FLAGS.actor:
        pygame.display.set_caption("Actor Window")  # 창 이름(제목) 설정
        ...
        pygame.display.set_caption("Learner Window")  # 창 이름(제목) 설정
        env = gym.make("box_picking_camera_env",
                    camera_mode="rgb",
                    max_episode_length=FLAGS.max_traj_length,
                    save_video=True,
    ...
    env = SERLObsInstructWrapper(env)
    ...
    # Setting Instruction Language Features
    language_keys = [key for key in env.observation_space.keys() if key == "instruction"]
    ...
    if FLAGS.learner:
        ...
        for demo_file in demo_files:
            if "fw" in demo_file:
                task_idx = 0
            elif "bw" in demo_file:
                task_idx = 1
            else:
                print(f"Unknown task in demo file: {demo_file}")
                #error
                continue
            with open(demo_file, "rb") as f:
                ...
                for traj in trajs:
                    for tra in traj:
                        ...
                        current_gripper_pos = tra['observations']['state']['gripper_state']
                        is_already_open = current_gripper_pos > 0.6  # 완전히 열린 상태 기준 (0.6 근처)
